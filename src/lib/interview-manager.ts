...
export async function processInterviewTurn(
  ...
  // 1. Evaluate student's answer grounded in the PDF reference
  const evaluation = await evaluateAnswerWithGroundTruth(
  ...
  if (isCompleted) {
    ...
    const conclusionSpoken = `Answer recorded. That concludes your technical interview. You covered ${coverageMetrics.coveredTopics} core topics. Your complete performance report is now ready.`;
  ...
  let adaptiveContext = `Student scored ${evaluation.scoreOutOf10}/10 on previous question ("${currentQuestion.questionText}").
Feedback was: "${evaluation.feedback}".
Recommendation: ${evaluation.adaptiveRecommendation}.`;

  if (evaluation.adaptiveRecommendation === 'SIMPLIFY_FOLLOW_UP') {
  ...
  } else if (evaluation.adaptiveRecommendation === 'INCREASE_DIFFICULTY') {
  ...
  // 6. Neutral acknowledgment without giving away correctness mid-interview
  // Full evaluation and score are presented in the final report after the exam
  const combinedSpokenText = `Answer recorded. Question ${nextIndex + 1}: ${nextQuestion.question}`;
  const coverageMetrics = calculateTopicCoverageMetrics(updatedCoverageMatrix);
