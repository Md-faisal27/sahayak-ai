    ...
    <main className="flex-1 max-w-4xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 border-b border-slate-200 dark:border-slate-800">
        ...
        {documents.length > 1 && (
          <div className="flex items-center gap-2">
            <span className="text-xs text-slate-500 font-mono">Select Document:</span>
            <select
              value={selectedDocId || ''}
              onChange={(e) => {
              ...
              className="text-xs bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg px-2.5 py-1.5 font-medium text-slate-800 dark:text-slate-200 focus:outline-hidden"
            >
              {documents.map((d) => (
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-mono font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-brand-50 dark:bg-brand-950 text-brand-700 dark:text-brand-300 border border-brand-200 dark:border-brand-900">
        ...
        <div className="flex flex-col sm:items-end gap-2 shrink-0">
          ...
          <div className="flex items-center gap-2 pt-1">
            <button
              ...
              className="px-2.5 py-1 text-[11px] font-mono border border-slate-200 dark:border-slate-800 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 flex items-center gap-1.5 transition-colors disabled:opacity-50"
            ...
            {flashcards.length > 0 && (
              <button
                ...
                className="px-2.5 py-1 text-[11px] font-mono border border-slate-200 dark:border-slate-800 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 flex items-center gap-1.5 transition-colors"
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div
            ...
            className="w-full min-h-[280px] sm:min-h-[340px] rounded-3xl bg-white dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-800 p-6 sm:p-8 shadow-sm cursor-pointer flex flex-col justify-between hover:border-brand-500 transition-all text-center relative overflow-hidden select-none"
          >
            <div className="flex items-center justify-between text-xs font-mono text-slate-500">
              <span>Card {currentIndex + 1} of {flashcards.length}</span>
              <span className="font-bold text-brand-700 dark:text-brand-400 uppercase tracking-wider">
                {currentCard?.topic}
              </span>
            ...
            <div className="flex items-center justify-between text-[11px] font-mono text-slate-400 pt-4 border-t border-slate-100 dark:border-slate-800/80">
              <span>Tip: Use Left/Right arrows to flip between cards</span>
              {currentCard?.known ? (
            ...
            {currentCard?.known && (
              <div className="absolute top-4 right-4 text-[10px] font-mono font-bold text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950 px-2.5 py-0.5 rounded-full border border-emerald-300 dark:border-emerald-800">
                ✓ Mastered
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-2 w-full sm:w-auto">
              <button
                ...
                className="flex-1 sm:flex-initial px-4 py-3 sm:py-2.5 border border-slate-200 dark:border-slate-800 rounded-xl text-xs font-mono font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors disabled:opacity-40 flex items-center justify-center gap-1.5"
              >
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto">
              <button
                ...
                className="flex-1 sm:flex-initial px-5 py-3 sm:py-2.5 bg-rose-50 dark:bg-rose-950/40 hover:bg-rose-100 dark:hover:bg-rose-950 text-rose-700 dark:text-rose-300 font-bold text-xs rounded-xl border border-rose-200 dark:border-rose-900 transition-all flex items-center justify-center gap-2"
              >
                <X className="w-4 h-4" /> Needs Practice
              </button>

              <button
                type="button"
                onClick={() => handleToggleKnown(true)}
                className="flex-1 sm:flex-initial px-5 py-3 sm:py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-xl shadow-xs transition-all flex items-center justify-center gap-2"
              >
                <Check className="w-4 h-4" /> I Know This Concept
              </button>
            </div>
          </div>
        </div>
